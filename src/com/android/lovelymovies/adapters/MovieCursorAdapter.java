...
public class MovieCursorAdapter extends CursorAdapter {
	...
	public long getItemId(int position) {
		long id = -1;
		if(mCursor != null) {
			final int prevPosition = mCursor.getPosition();
			mCursor.moveToPosition(position);
			id = mCursor.getLong(mCursor.getColumnIndex(Movies.MOVIE_ID));
	...
	public void bindView(View view, Context context, Cursor cursor) {
		...
		mImageLoader.displayImage(cursor.getString(cursor.getColumnIndex(Movies.MOVIE_IMGURL)), holder.image, mOptions);
	}
